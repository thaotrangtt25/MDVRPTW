...
# Kết thúc tại kho
    ...
    # If the route is empty, assign the customer with the closest ready time to the current time for the vehicle.
        if len(self.service_route) == 1:
            return self.serve_customer(new_customer) or self.serve_customer_force(new_customer)

    # Sort the service route by the ready time of the customers.
        sorted_route = sorted(self.service_route[1:], key=lambda node: node[0].ready_time)

    # Find the index where the ready time of the customer is greater than the last service time.
        index = next((i for i, node in enumerate(sorted_route) if node[0].ready_time > self.last_service_time), None)

        if index is not None:
            # Try to serve the new customer at the found index.
            ...
            for customer, curr_time in sorted_route[:index]:
                if not vehicle.serve_customer(customer):
                    if not vehicle.serve_customer_force(customer):
                        should_use_route = False
                        break

            if should_use_route and (vehicle.serve_customer(new_customer) or vehicle.serve_customer_force(new_customer)):
...
class Instance:
    ...
    def sort_by_ready_time(self):
        # sorting the customer list in ascending order
            ...
            while True:
                ...
                if not found:
                    for customer in self.customer_list:
                        ...
                        if v.try_to_serve_customer(customer):
                            found = True
                            break
                if not found:
                    break
